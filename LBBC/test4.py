...
for level in range(6, 8):
    size = 0
    mul = 10
    for ext in range(mul):
        size += 1000
        print('----------------------------------')
        print(f'level={level} block编号= {ext} size = {size}')
        test_data_list.append(f'{level},{ext},{size},{block(level, size)}')
f = open('simu_test1.txt', mode='w', encoding='utf-8')
f.writelines('\n'.join(test_data_list))
f.close()
